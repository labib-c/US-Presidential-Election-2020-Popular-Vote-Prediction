...
The model chosen to model the proportion of voters who will vote for Donald Trump in the 2020 United States Presidential Election is a Logistic Regression model represented by the following equation (Eq. 1):
$$
\log({\frac{p}{1-p}}) = \beta_0 + \sum^{K}_{i=1}\beta_ix_i
$$

The response variable $p$ indicates whether someone votes for Donald Trump. $\beta_0$ represents the intercept term, which represents the case where every factor level in the chosen predictors is at the initial level. Similarly, $\sum^{K}_{i=1}\beta_ix_i$ indicates the slope $\beta_i$ associated with each factor level of the predictors $x_i$. This conveys that the slope associated with each factor level represents the change in log odds for every unit increased in the associated predictor. Also, $K$ represents the total number of factor levels in each predictor.
The chosen predictors for our model are age, race, household income, and state. 

A Logistic Model was chosen for this problem space due to the binary nature of the response variable `vote_trump`, where `1` represents a vote for Donald Trump and `0` represents a vote for Joe Biden (all other vote options were removed such as ‘Will not vote’). The predictors were selected as we believe these are key factors when one decides which candidate to vote for, and we wanted to explore whether or not these factors are as significant as we believe them to be.
The model from Eq. 1 is fit using survey data from the voter study group data collected by UCLA and Democracy Fund (Tausanovitch and Vavreck, 2020) .
...
According to our post-stratification analysis of the proportion of voters in favour of Trump modelled by a Logistical Regression model we think that the proportion of voters who will favour Trump in the upcoming election is 0.507. Our model is based on key demographics of an individual such as, age of the individual, state of residence, their income category and their race.
Studying the census data (Figure 1) we found a sharp division in the ‘age_cat’ variable. Only 36% of individuals aged 18-25 were in favour of voting for Trump compared to 61% of individuals aged 85 and over. We also observed a linear trend in ‘age_cat’ with respect to voting in favour of Trump. As a result, if an individual is aged 85 or over then the probability that they vote for Trump increases roughly by a factor of 1.07 compared to a factor of 0.41 if the individual is aged 18-25 (Table 1).
...
census_data %>%
  ...
  summarise(alp_predict = sum(alp_predict_prop)/sum(n))
```
(Figure 1: Prediction of Proportion of Trump Votes by Age Category)
...
census_data %>%
  ...
  summarise(alp_predict = sum(alp_predict_prop)/sum(n))
```
(Figure 2: Prediction of Proportion of Trump Votes by Race)

We also found individuals who identified as ‘White’ and ‘American Indian or Alaska native’ were more likely to vote for Trump compared to individuals from the ‘African American’ community (Figure 2).
From Table 1, we observe the slope for the “black/african american/negro” race level has a value of -2.28, which is one of the steepest decrease from our model. This shows that the log odds of a voter voting for Trump is they are African American decreases by a factor of 2.28, which confirms the prediction of our model described previously.


# Discussion

As we are trying to model the proportion of voters who will vote for Donald Trump, we proceeded with a logistic regression model as the response variable we are looking for is binary: 1 representing a vote for Donald Trump, and 0 representing a vote for Joe Biden. This model was trained on four predictor variables: age, household income, residing state, and race. We then employ the post-stratification technique by partitioning the data we retrieved from the Integrated Public Use Microdata Series (IPUMS) into their own prospective household income group, and then applying the logistic model to the dataset obtained from IPUMS.
